    ...
    <footer className="bg-brand-950 text-white">
      <div className="mx-auto grid max-w-7xl gap-12 px-4 py-14 sm:px-6 lg:grid-cols-[1.2fr_.8fr_.8fr_1fr] lg:px-8">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-white/55">Home Tuition</p>
            </div>
          </div>
          <p className="mt-5 max-w-sm text-sm leading-7 text-white/65">
            Personalised academic support at home, designed to help students learn with more clarity, confidence and consistency.
          </p>
        ...
        <div>
          ...
          <div className="mt-4 grid gap-3">
            {quickLinks.map(([label, href]) => (
              <Link key={href} to={href} className="text-sm text-white/60 transition hover:text-white">
                {label}
              </Link>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="mt-4 grid gap-3">
            {courseLinks.map((label) => (
              <Link key={label} to="/courses" className="text-sm text-white/60 transition hover:text-white">
                {label}
              </Link>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="mt-4 flex rounded-2xl border border-white/10 bg-white/5 p-1.5">
            ...
            <button type="button" aria-label="Subscribe to newsletter" className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-accent-500 text-brand-950">
              <Send size={17} />
            </button>
          </div>
      ...
      <div className="border-t border-white/10 px-4 py-5 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 text-xs text-white/40 sm:flex-row sm:items-center sm:justify-between">
          <span>© {new Date().getFullYear()} Reliant India Group. All rights reserved.</span>
          <span className="inline-flex items-center gap-1">Built for better learning <ArrowUpRight size={13} /></span>
        </div>
      </div>
    </footer>
  );
